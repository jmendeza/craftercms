...
compilePackage() {
  ...
  if containsElement "${3}" "${TSC_PACKAGES[@]}"; then
    toRelative "======      [${3}]: COMPILING: ${TSC} -p ${1}/tsconfig-build.json"
    $TSC -p ${1}/tsconfig-build.json
  else
    toRelative "======      [${3}]: COMPILING: ${NGC} -p ${1}/tsconfig-build.json"
...
#  toRelative "====== (compiler)COMPILING: \$(npm bin)/tsc -p packages/compiler/tsconfig-tools.json"
#  $(npm bin)/tsc -p packages/compiler/tsconfig-tools.json
#  toRelative "====== (compiler)COMPILING: \$(npm bin)/tsc -p packages/compiler-cli/tsconfig-tools.json"
...
for PACKAGE in ${PACKAGES[@]}; do
  ...
  if [[ ${BUNDLE} == true ]]; then
    ...
    if ! containsElement "${PACKAGE}" "${NODE_PACKAGES[@]}"; then
      ...
      toRelative "======        Copy ${PACKAGE} typings"
      ...
      (
        ...
        toRelative "======         Run rollup conversions on ${PACKAGE}"
        runRollup ${SRC_DIR}
        addBanners ${BUNDLES_DIR}
        minify ${BUNDLES_DIR}

        if [[ -e ${SRC_DIR}/build.sh ]]; then
          toRelative "======         Custom build for ${PACKAGE}"
      ...
      if [[ ${PACKAGE} == "common" ]]; then
        toRelative "======      Copy i18n locale data"
        rsync -a ${OUT_DIR}/locales/ ${NPM_DIR}/locales
      fi
    else
      toRelative "======        Copy ${PACKAGE} node tool"
      rsync -a ${OUT_DIR}/ ${NPM_DIR}
    fi

    toRelative "======        Copy ${PACKAGE} package.json and .externs.js files"
